...
%
if dd >= 3, sprintf('Needs at least %g Gbyte RAM', prod(Jd)*M*8/10^9*2), end
...
% Jiaen's modification
% ----------------------
% $$$ krad=cell(dd,1);
% $$$ if dd==1
% $$$     kdim=circshift([0:Kd(1)-1]-floor(Kd(1)/2),-floor(Kd(1)/2))/Kd(1)*2*pi;
% $$$     krad{1}=kdim(:);
% $$$     clear kdim
% $$$ elseif dd==2
% $$$     kdim1=circshift([0:Kd(1)-1]-floor(Kd(1)/2),-floor(Kd(1)/2))/Kd(1)*2*pi;
% $$$     kdim2=circshift([0:Kd(2)-1]-floor(Kd(2)/2),-floor(Kd(2)/2))/Kd(2)*2*pi;
% $$$ % $$$     kdim1=([0:Kd(1)-1]-floor(Kd(1)/2))/Kd(1)*2*pi;
% $$$ % $$$     kdim2=([0:Kd(2)-1]-floor(Kd(2)/2))/Kd(2)*2*pi;
% $$$     [kdim1,kdim2]=ndgrid(kdim1,kdim2);
% $$$     krad{1}=kdim1(:);
% $$$     krad{2}=kdim2(:);
% $$$     clear kdim1  kdim2
% $$$ elseif dd==3
% $$$     kdim1=circshift([0:Kd(1)-1]-floor(Kd(1)/2),-floor(Kd(1)/2))/Kd(1)*2*pi;
% $$$     kdim2=circshift([0:Kd(2)-1]-floor(Kd(2)/2),-floor(Kd(2)/2))/Kd(2)*2*pi;
% $$$     kdim3=circshift([0:Kd(3)-1]-floor(Kd(3)/2),-floor(Kd(3)/2))/Kd(3)*2*pi;
% $$$ % $$$     kdim1=([0:Kd(1)-1]-floor(Kd(1)/2))/Kd(1)*2*pi;
% $$$ % $$$     kdim2=([0:Kd(2)-1]-floor(Kd(2)/2))/Kd(2)*2*pi;
% $$$ % $$$     kdim3=([0:Kd(3)-1]-floor(Kd(3)/2))/Kd(3)*2*pi;
% $$$     [kdim1,kdim2,kdim3]=ndgrid(kdim1,kdim2,kdim3);
% $$$     krad{1}=kdim1(:);
% $$$     krad{2}=kdim2(:);
% $$$     krad{3}=kdim3(:);
% $$$     clear kdim1 kdim2 kdim3
% $$$ end
% $$$ nJ=prod(Jd);
% $$$ phase=zeros(nJ,M,dd);
% $$$ for id=1:dd
...
% $$$ end
% $$$ clear krad
% $$$ phase=reshape(phase,[nJ*M,dd]);
% $$$ phase=exp(1i*((phase)*n_shift(:)));
% $$$ phase=reshape(phase,[nJ,M]);
% $$$ uu = conj(uu) .* phase;		% [*Jd,M]
% $$$ clear phase
% ----------------------
% original nufft
% ----------------------
% [1,M]
% phase = exp(1i * (wrapToPi(om) * n_shift(:))).';
phase = exp(1i * ((om) * n_shift(:))).';
uu = conj(uu) .* phase(ones(1,prod(Jd)),:);		% [*Jd,M]
% ----------------------

mm = [1:M]; mm = mm(ones(prod(Jd),1),:);		% [*Jd,M]
st.p = sparse(double(mm(:)), double(kk(:)), double(uu(:)), M, prod(Kd));	% sparse matrix
% $$$ st.prow=mm(:);
% $$$ st.pcol=kk(:);
% $$$ st.pval=uu(:);
% $$$ st.pm=M;
