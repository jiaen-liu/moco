function p=get_coordinate(para,oversamp,steref,nonsiemens)
    ...
    pr=reshape(([0:nr-1]-(nr-1)/2)*resr,[nr,1]);
    pp=reshape(([0:np-1]-(np-1)/2)*resp,[1,np]);
    part_shift=([0:npar-1]-(npar-1)/2)*ress;
    for is=1:para.n_slices
        new_coor=gen_coordinate(para.snormal(:, is), para.prot);
        for ipart=1:npar
            tmp=pr.*permute(new_coor(:,1),[2,3,4,1])+...
                pp.*permute(new_coor(:,2),[2,3,4,1]);
            p(:,:,(is-1)*npar+ipart,:)=tmp+...
                permute(part_shift(ipart)*new_coor(:,3),...
                        [2,3,4,1]);
